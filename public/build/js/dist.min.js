...
}]),angular.module("template/typeahead/typeahead-popup.html",[]).run(["$templateCache",function($templateCache){$templateCache.put("template/typeahead/typeahead-popup.html",'<ul class="dropdown-menu" ng-if="isOpen()" ng-style="{top: position.top+\'px\', left: position.left+\'px\'}" style="display: block;" role="listbox" aria-hidden="{{!isOpen()}}">\n    <li ng-repeat="match in matches track by $index" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)" role="option" id="{{match.id}}">\n        <div typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></div>\n    </li>\n</ul>')}]),angular.element(document).ready(function(){"#_=_"===window.location.hash&&(window.location.hash="#!"),angular.bootstrap(document,["mean"])});var packageModules=[];for(var index in window.modules)angular.module(window.modules[index].module,window.modules[index].angularDependencies||[]),packageModules.push(window.modules[index].module);var modules=["ngCookies","ngResource","ngRoute","ngAnimate","ngAutocomplete","xngClearable","geolocation","ui.bootstrap","ui.router","mean.system","mean.auth"];modules=modules.concat(packageModules),angular.module("mean",modules),angular.module("mean.auth",[]),angular.module("mean.system",["mean.controllers.login","mean-factory-interceptor"]),angular.module("mean.controllers.login",[]).controller("LoginCtrl",["$scope","$rootScope","$http","$location",function($scope,$rootScope,$http,$location){$scope.user={},$scope.login=function(){$http.post("/login",{email:$scope.user.email,password:$scope.user.password}).success(function(response){$scope.loginError=0,$rootScope.user=response.user,$rootScope.$emit("loggedin"),response.redirect?window.location.href===response.redirect?window.location.reload():window.location=response.redirect:$location.url("/")}).error(function(){$scope.loginerror="Authentication failed."})}}]).controller("RegisterCtrl",["$scope","$rootScope","$http","$location",function($scope,$rootScope,$http,$location){$scope.user={},$scope.register=function(){$scope.usernameError=null,$scope.registerError=null,$http.post("/register",{email:$scope.user.email,password:$scope.user.password,confirmPassword:$scope.user.confirmPassword,username:$scope.user.username,name:$scope.user.name}).success(function(){$scope.registerError=0,$rootScope.user=$scope.user,$rootScope.$emit("loggedin"),$location.url("/")}).error(function(error){"Username already taken"===error?$scope.usernameError=error:$scope.registerError=error})}}]),angular.module("mean.auth").config(["$stateProvider",function($stateProvider){var checkLoggedOut=function($q,$timeout,$http,$location){var deferred=$q.defer();return $http.get("/loggedin").success(function(user){"0"!==user?($timeout(deferred.reject),$location.url("/login")):$timeout(deferred.resolve)}),deferred.promise};$stateProvider.state("auth.login",{url:"/login",templateUrl:"public/auth/views/login.html",resolve:{loggedin:checkLoggedOut}}).state("auth.register",{url:"/register",templateUrl:"public/auth/views/register.html",resolve:{loggedin:checkLoggedOut}})}]),angular.module("mean.system").controller("HeaderController",["$scope","$rootScope","Global","Menus",function($scope,$rootScope,Global,Menus){function queryMenu(name,defaultMenu){Menus.query({name:name,defaultMenu:defaultMenu},function(menu){$scope.menus[name]=menu})}$scope.global=Global,$scope.menus={};var defaultMainMenu=[];queryMenu("main",defaultMainMenu),$scope.isCollapsed=!1,$rootScope.$on("loggedin",function(){queryMenu("main",defaultMainMenu),$scope.global={authenticated:!!$rootScope.user,user:$rootScope.user}})}]),angular.module("mean.system").controller("IndexController",["$log","$scope","$rootScope","$location","Global","Places","geolocation",function($log,$scope,$rootScope,$location,Global,Places,geolocation){$scope.global=Global,Places.getQuery().location||geolocation.getLocation().then(function(data){$scope.coords={lat:data.coords.latitude,lng:data.coords.longitude},Places.setQuery({latlng:[$scope.coords.lat,$scope.coords.lng].join(","),location:"Current Location"}),$log.info($scope.coords)})}]),angular.module("mean.system").controller("PlacesController",["$log","$scope","$rootScope","$http","$routeParams","$location","Global","Menus","Places","geolocation",function($log,$scope,$rootScope,$http,$routeParams,$location,Global,Menus,Places){$scope.global=Global,$scope.init=function(){Places.get($location.search()).then(function(){})},$scope.init()}]),angular.module("mean.system").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("home",{url:"/",templateUrl:"public/system/views/index.html"}).state("auth",{templateUrl:"public/auth/views/index.html"}).state("places",{url:"/places",templateUrl:"public/system/views/places.html",reloadOnSearch:!0,controller:"PlacesController"})}]).config(["$locationProvider",function($locationProvider){$locationProvider.hashPrefix("!")}]),angular.module("mean.system").factory("Global",[function(){var _this=this;return _this._data={user:window.user,authenticated:!1,isAdmin:!1},window.user&&window.user.roles&&(_this._data.authenticated=window.user.roles.length,_this._data.isAdmin=~window.user.roles.indexOf("admin")),_this._data}]),angular.module("mean-factory-interceptor",[]).factory("httpInterceptor",["$q","$location",function($q,$location){return{response:function(response){return 401===response.status?($location.path("/login"),$q.reject(response)):response||$q.when(response)},responseError:function(rejection){return 401===rejection.status?($location.url("/login"),$q.reject(rejection)):$q.reject(rejection)}}}]).config(["$httpProvider",function($httpProvider){$httpProvider.interceptors.push("httpInterceptor")}]),angular.module("mean.system").factory("Menus",["$resource",function($resource){return $resource("admin/menu/:name",{name:"@name",defaultMenu:"@defaultMenu"})}]),angular.module("mean.system").service("Places",["$log","$resource","$http","$location","$q",function($log,$resource,$http,$location,$q){var savedResults={},savedQuery={},fetching=!1;return{getData:function(){return savedResults},setData:function(results){$log.info("Data set:",results),savedResults=results},clearData:function(){savedResults={}},getQuery:function(){return savedQuery},setQuery:function(query){$log.info("Query set:",query),_.forOwn(query,function(v,k){$location.search(k,v)}),savedQuery=query},clearQuery:function(){savedQuery={}},fetching:function(val){return $log.info("Fetching:",val),fetching=val,this.fetched()},fetched:function(){return fetching},get:function(query){this.setQuery(query),this.fetching(!0);var deferred=$q.defer();query="Current Location"===query.location?_.omit(query,["location"]):_.omit(query,["latlng"]);var promise=$http({url:"/api/1/places",method:"POST",data:query});return promise.then(function(data){return $log.debug(data),this.setData(data.data),this.fetching(!1),_.has(data.data,"places")&&0===data.data.places.length?deferred.reject(new Error("No more results")):deferred.resolve(data)}.bind(this),function(err){return this.fetching(!1),deferred.reject(err)}.bind(this)),deferred.promise}}}]),angular.module("mean.system").directive("btPlaces",["$log","$rootScope","Places",function($log,$rootScope,Places){return{templateUrl:"/public/system/directives/places/places.tpl.html",restrict:"A",scope:{},controller:function(){},link:function($scope){function success(){window.scrollTo(0,1)}function fail(err){$log.error(err.message)}$scope.next=function(){var query=Places.getQuery();Places.get(_.extend(query,{page:_.parseInt(query.page)+1})).then(success,fail)},$scope.prev=function(){var query=Places.getQuery();Places.get(_.extend(query,{page:_.parseInt(query.page)-1})).then(success,fail)},$scope.$watch(function(){return Places.getData()},function(newVal){$scope.places=newVal,$scope.page=Places.getQuery().page,$scope.hasPagination=!(!$scope.places.places||!$scope.places.places.length),$scope.hasNext=!!($scope.page>0),$scope.hasPrev=!!($scope.page>1)}),$scope.$watch(function(){return Places.getQuery()},function(newVal){_.extend($scope,$scope,newVal)}),$scope.$watch(function(){return Places.fetched()},function(newVal){_.extend($scope,$scope,{fetching:newVal})})}}}]),angular.module("mean.system").directive("btSearchForm",["$log","$rootScope","$location","Places",function($log,$rootScope,$location,Places){return{templateUrl:"/public/system/directives/search-form/search-form.tpl.html",restrict:"A",scope:{},controller:function(){},link:function($scope,element,attrs){var options={country:"us",types:["(regions)"]},input=document.getElementById("autocomplete"),autocomplete=new google.maps.places.Autocomplete(input,options);google.maps.event.addListener(autocomplete,"place_changed",function(){$log.debug(autocomplete.getPlace()),$scope.location=input.value}),$scope.centered=attrs.centered,$scope.send=function(){$log.info("Sending request"),$scope.page=1;var query={location:$scope.location,page:$scope.page};Places.get(query).then(function(data){$log.info(data),$location.path("/places")},function(err){$log.error(err)})},$scope.submitForm=function(isValid){$scope.submitted=!0,isValid=!0,isValid&&$scope.send()},$scope.$watch(function(){return Places.getQuery()},function(newVal){$log.info("getQuery",newVal),_.extend($scope,$scope,newVal)})}}}]);